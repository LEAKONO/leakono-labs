...
import { ChevronDown, ChevronUp, Monitor, Server, Database, Shield } from 'lucide-react';
import { FaGithub } from 'react-icons/fa';
import type { SoftwareProject } from '../../types';

const ARCH_LAYERS = [
  { key: 'frontend', label: 'Frontend', icon: Monitor, color: '#38bdf8' },
  { key: 'backend', label: 'Backend', icon: Server, color: '#10b981' },
  { key: 'database', label: 'Database', icon: Database, color: '#8b5cf6' },
  { key: 'auth', label: 'Auth', icon: Shield, color: '#f59e0b' },
];

const FLOW_STEPS = ['Client', 'API Layer', 'Auth', 'Controller', 'Service', 'Database'];
    ...
    >
      <div className="flex items-center justify-between px-5 py-3 border-b border-white/5 bg-white/[0.02]">
        ...
        >
          ...
          />
          <span className="font-mono text-[10px] text-emerald-400 font-semibold">DEPLOYED</span>
        </div>
      </div>
      ...
      <div className="p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-5">
          <div className="flex-1">
            ...
            <h3 className="text-white text-xl sm:text-2xl font-bold tracking-tight mt-1 mb-2">
              {project.title}
            </h3>
        ...
        >
          {expanded
            ? <><ChevronUp size={14} /> Hide Architecture</>
            : <><ChevronDown size={14} /> Inspect Architecture</>
          }
        </button>
                  ...
                  {ARCH_LAYERS.map((layer) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-2 rounded-lg font-mono text-[11px] tracking-wide transition-all duration-200"
                        style={{
                          background: isActive ? `${layer.color}15` : 'rgba(255,255,255,0.02)',
                          border: isActive ? `1px solid ${layer.color}40` : '1px solid rgba(255,255,255,0.05)',
                    ...
                    <div className="space-y-2">
                      {currentItems.map((item, i) => (
                        ...
                        >
                          ...
                          <div
                            className="flex-1 px-3 py-2 rounded-lg font-mono text-xs text-white/80 border border-white/5"
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {FLOW_STEPS.map((step, i) => (
                      <div key={step} className="flex items-center gap-2">
                        <div
                          className="px-2.5 py-1.5 rounded-lg font-mono text-[10px] text-[#8b949e] whitespace-nowrap"
                        ...
                        {i < FLOW_STEPS.length - 1 && (
                          <div className="relative w-6 h-px bg-cyan-glow/20 shrink-0">
                            <motion.div
                              ...
                              className="absolute top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-cyan-glow"
                              style={{ left: 0 }}
                            />
                          </div>
                        )}
